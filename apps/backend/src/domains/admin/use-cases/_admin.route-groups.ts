...
import { getBillingStatus, applyRolesAndEntitlements, upsertSubscription } from '../../../services/billing';
...
settingsRouter.get('/me', async (req, res) => {
  ...
  const user = await adminRepository.user.findUnique({ where: { id: userId } });
  if (!user) {
...
async function countUsersByRole(role: UserRole) {
  return adminRepository.user.count({ where: { roles: { has: role } } });
}
...
usersRouter.get('/overview', async (req, res) => {
  const [totalUsers, totalFree, totalPremium, totalVip, totalActiveSubs, exports24h] = await Promise.all([
    adminRepository.user.count(),
    countUsersByRole('user'),
    countUsersByRole('premium'),
    countUsersByRole('vip'),
    adminRepository.subscription.count({ where: { status: SubscriptionStatus.active } }),
    adminRepository.gdprExport.count({ where: { requestedAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } } })
  ]);

  const signups7 = await buildTimeseries(7, (from, to) =>
    adminRepository.user.count({ where: { createdAt: { gte: from, lt: to } } })
  );

  const upgrades7 = await buildTimeseries(7, (from, to) =>
    adminRepository.roleGrant.count({ where: { grantedAt: { gte: from, lt: to }, role: { in: ['premium', 'vip'] } } })
  );

  const churn7 = await buildTimeseries(7, (from, to) =>
    adminRepository.subscription.count({ where: { status: SubscriptionStatus.canceled, updatedAt: { gte: from, lt: to } } })
  );
...
usersRouter.get('/users', async (req, res) => {
  ...
  const users = await adminRepository.user.findMany({
    where,
...
usersRouter.get('/users/:id', async (req, res) => {
  ...
  const sessionsCount = await adminRepository.session.count({ where: { userId: user.id, revokedAt: null } });
...
usersRouter.post('/users/:id/reveal', async (req, res) => {
...
usersRouter.patch('/users/:id/role', async (req, res) => {
  ...
  const target = await adminRepository.user.findUnique({ where: { id: req.params.id } });
  if (!target) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
  }
...
usersRouter.patch('/users/:id/status', async (req, res) => {
...
securityRouter.post('/users/:id/password/reset', async (req, res) => {
  ...
  await adminRepository.passwordResetToken.create({
    data: {
      userId: user.id,
      tokenHash,
      expiresAt
...
securityRouter.post('/users/:id/email/verify/force', async (req, res) => {
  const user = await adminRepository.user.findUnique({ where: { id: req.params.id } });
  if (!user) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
...
securityRouter.post('/users/:id/email/verify/revoke', async (req, res) => {
  const user = await adminRepository.user.findUnique({ where: { id: req.params.id } });
  if (!user) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
...
securityRouter.post('/users/:id/sessions/revoke', async (req, res) => {
  ...
  if (mode === 'current') {
    const latest = await adminRepository.session.findFirst({
      where: { userId: user.id, revokedAt: null },
      orderBy: { lastUsedAt: 'desc' }
    });
    if (latest) {
      await adminRepository.session.update({ where: { id: latest.id }, data: { revokedAt: new Date() } });
    }
  } else {
    await revokeAllSessions(user.id);
  }
...
exportsRouter.post('/users/:id/gdpr/export', async (req, res) => {
  const user = await adminRepository.user.findUnique({ where: { id: req.params.id } });
  if (!user) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
...
exportsRouter.post('/users/:id/delete', async (req, res) => {
  const user = await adminRepository.user.findUnique({ where: { id: req.params.id } });
  if (!user) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
...
exportsRouter.post('/users/:id/purge', async (req, res) => {
  const deletion = await adminRepository.deletionRequest.findFirst({
...
billingRouter.get('/plans', async (req, res) => {
  const plans = await adminRepository.plan.findMany({ orderBy: { amountCents: 'asc' } });
  res.json({
...
billingRouter.post('/plans', requireSuperAdmin, async (req, res) => {
  ...
  const plan = await adminRepository.plan.create({
    data: {
...
billingRouter.patch('/plans/:planId', requireSuperAdmin, async (req, res) => {
  ...
  const plan = await adminRepository.plan.findUnique({ where: { id: planId } });
  if (!plan) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
  }
  ...
  await adminRepository.plan.update({ where: { id: plan.id }, data: updates });
...
billingRouter.post('/stripe/coupons', requireSuperAdmin, async (req, res) => {
...
billingRouter.post('/users/:id/subscription/change', async (req, res) => {
  ...
  const plan = await adminRepository.plan.findFirst({ where: { code: parseResult.data.plan_code as PlanCode } });
  if (!plan) {
    res.status(404).json({ error: 'PLAN_NOT_FOUND', request_id: req.id });
    return;
  }

  const existing = await adminRepository.subscription.findFirst({ where: { userId: user.id }, orderBy: { updatedAt: 'desc' } });
  ...
  if (!env.isTest) {
    ...
    if (stripeSubscriptionId) {
      ...
      const customer = await adminRepository.stripeCustomer.findUnique({ where: { userId: user.id } });
      if (!customer) {
...
billingRouter.get('/users/:id/credits', async (req, res) => {
...
billingRouter.post('/users/:id/credits/adjust', async (req, res) => {
...
auditRouter.get('/audit', async (req, res) => {
  ...
  const logs = await adminRepository.auditLog.findMany({
    where,
...
exportsRouter.get('/exports', async (req, res) => {
  ...
  const exports = await adminRepository.gdprExport.findMany({
    where,
...
export const adminRouteRouters = {
  usersRouter,
  securityRouter,
  billingRouter,
  auditRouter,
