...
async function ensureRecentMfaIfEnabled(userId: string): Promise<boolean> {
  const user = await meRepository.user.findUnique({
    where: { id: userId },
    select: { mfaEnabled: true }
  });
  if (!user || !user.mfaEnabled) return true;

  const factor = await meRepository.mfaFactor.findFirst({
...
async function ensureUsernameAvailable(params: { userId: string; username: string }) {
  const existing = await meRepository.user.findFirst({
    where: {
...
profileRouter.get('/onboarding', requireAuth, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) {
    res.status(401).json({ error: 'AUTH_REQUIRED', request_id: req.id });
    return;
...
profileRouter.patch('/onboarding', requireAuth, async (req, res) => {
...
avatarRouter.post('/avatar/upload-url', requireAuth, avatarLimiter, async (req, res) => {
...
avatarRouter.post('/avatar/confirm', requireAuth, async (req, res) => {
...
gdprRouter.post('/gdpr/export/request', requireAuth, requireRecentMfa, gdprExportLimiter, async (req, res) => {
  ...
  const user = await meRepository.user.findUnique({ where: { id: userId } });
  if (!user) {
    res.status(404).json({ error: 'NOT_FOUND', request_id: req.id });
    return;
  }
...
gdprRouter.get('/gdpr/export/:id/status', requireAuth, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) {
    res.status(401).json({ error: 'AUTH_REQUIRED', request_id: req.id });
    return;
  ...
  const exportRecord = await meRepository.gdprExport.findFirst({
...
gdprRouter.get('/gdpr/export/:id/download-url', requireAuth, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) {
    res.status(401).json({ error: 'AUTH_REQUIRED', request_id: req.id });
    return;
...
gdprRouter.post('/gdpr/delete/request', requireAuth, requireRecentMfa, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) {
    res.status(401).json({ error: 'AUTH_REQUIRED', request_id: req.id });
    return;
...
profileRouter.post('/consents', requireAuth, async (req, res) => {
...
sessionsRouter.get('/sessions', requireAuth, async (req, res) => {
  ...
  const currentSession = refreshHash
    ? await meRepository.session.findUnique({ where: { refreshTokenHash: refreshHash } })
    : null;

  const sessions = await meRepository.session.findMany({
...
sessionsRouter.get('/sessions/history', requireAuth, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) {
    res.status(401).json({ error: 'AUTH_REQUIRED', request_id: req.id });
    return;
...
sessionsRouter.post('/sessions/revoke', requireAuth, async (req, res) => {
  ...
  const session = await meRepository.session.findFirst({
...
sessionsRouter.post('/sessions/revoke-all', requireAuth, async (req, res) => {
  ...
  if (includeCurrent) {
    ...
    await meRepository.session.updateMany({
...
securityRouter.post('/mfa/backup-codes/regenerate', requireAuth, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) {
    res.status(401).json({ error: 'AUTH_REQUIRED', request_id: req.id });
    return;
  ...
  await meRepository.$transaction([
    meRepository.backupCode.deleteMany({ where: { userId } }),
    meRepository.backupCode.createMany({ data: backupCodeHashes })
  ]);
...
securityRouter.post('/security/alerts', requireAuth, async (req, res) => {
  ...
  await meRepository.user.update({
    where: { id: userId },
    data: {
...
emailRouter.post('/recovery-email', requireAuth, recoveryEmailLimiter, async (req, res) => {
  ...
  const existingRecovery = await meRepository.user.findFirst({
    where: {
      recoveryEmail,
      id: { not: userId }
    },
  ...
  await meRepository.$transaction([
    meRepository.recoveryEmailToken.deleteMany({ where: { userId } }),
    meRepository.recoveryEmailToken.create({
      data: {
        userId,
        email: recoveryEmail,
        tokenHash,
...
emailRouter.delete('/recovery-email', requireAuth, async (req, res) => {
...
securityRouter.post('/password', requireAuth, async (req, res) => {
...
emailRouter.post('/email', requireAuth, async (req, res) => {
  ...
  const existing = await meRepository.user.findUnique({ where: { email: newEmail } });
  if (existing) {
    res.status(409).json({
  ...
  res.json({
    ok: true,
    request_id: req.id,
    ...(env.isTest ? { test_token: token } : {})
  });
});

export const meRouteRouters = {
  profileRouter,
  securityRouter,
  sessionsRouter,
  gdprRouter,
