...
router.post('/email/resend', resendLimiter, async (req, res) => {
...
router.get('/recovery-email/verify', async (req, res) => {
  const token = typeof req.query.token === 'string' ? req.query.token : null;
  if (!token) {
    res.status(400).json({ error: 'TOKEN_INVALID', request_id: req.id });
    return;
...
router.get('/security/revoke-sessions', async (req, res) => {
  const token = typeof req.query.token === 'string' ? req.query.token : null;
  if (!token) {
    res.status(400).json({ error: 'TOKEN_INVALID', request_id: req.id });
    return;
...
router.get('/security/acknowledge-alert', async (req, res) => {
  const token = typeof req.query.token === 'string' ? req.query.token : null;
  if (!token) {
    res.status(400).json({ error: 'TOKEN_INVALID', request_id: req.id });
    return;
...
router.post('/phone/check', phoneCheckLimiter, async (req, res) => {
  const accessToken = req.cookies?.[ACCESS_COOKIE_NAME] as string | undefined;
  let userId: string | null = null;
...
router.post('/mfa/setup/start', async (req, res) => {
  const accessToken = req.cookies?.[ACCESS_COOKIE_NAME] as string | undefined;
  let userId: string | null = null;
...
router.get('/email/change/verify', async (req, res) => {
  ...
  try {
    ...
    await prisma.user.update({
      ...
      data: {
        email: newEmail,
        // Also mark as verified since they clicked the link sent to that email
        emailVerifiedAt: new Date()
      }
    });
