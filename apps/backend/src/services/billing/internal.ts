  ...
  if (targetPlanCode === PlanCode.FREE) {
    ...
    if (env.isTest) {
      return {
        ...
        message: 'Plan gratuit appliqué immédiatement.'
      };
    }

    // Immediate cancellation; entitlement/role transition is applied by webhook handlers.
    await stripe.subscriptions.cancel(currentSubscription.stripeSubscriptionId);

    return {
      changeType: 'downgrade',
      effectiveAt: new Date(),
      message: 'Plan gratuit demandé. Application immédiate après confirmation Stripe.'
    };
  }
  ...
  try {
    ...
    if (changeType === 'upgrade') {
      ...
      return {
        changeType: 'upgrade',
        effectiveAt: new Date(),
        message: 'Plan mis à jour immédiatement. Synchronisation finale via webhook Stripe.'
      };
    } else {
      // DOWNGRADE: Immediate without prorata
      await stripe.subscriptions.update(stripeSubscriptionId, {
        items: [{
          id: currentItemId,
          price: planWithPrice.stripePriceId
      ...
      logger.info({ userId, from: currentPlanCode, to: targetPlanCode }, 'Plan downgraded');

      return {
        changeType: 'downgrade',
        effectiveAt: new Date(),
        message: 'Plan rétrogradé immédiatement. Synchronisation finale via webhook Stripe.'
      };
    }
  } catch (error) {
