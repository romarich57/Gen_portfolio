...
import { verifyOAuthLink } from '@/api/auth';
import type { ApiError } from '@/api/http';

function VerifyOAuthLink() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="text-center">
        ...
        <h1 className="mt-3 text-3xl font-display font-semibold">Approbation de la liaison OAuth</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Cette action autorise un provider OAuth a se connecter a votre compte.
        </p>
      </div>
          ...
          {status === 'loading' && (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Nous finalisons la liaison OAuth.</p>
              </div>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold">Liaison approuvee</p>
                  <p className="text-xs text-muted-foreground">
                    Vous pouvez maintenant utiliser ce provider OAuth pour vous connecter.
                  </p>
                </div>
              </div>
...
export default VerifyOAuthLink;
